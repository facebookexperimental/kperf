				...
				__u64 offset, __u32 pat_start, __u32 size)
{
	...
	ret = cudaMemcpy(rxbuf, (void *)(mem->buf_mem + offset), size, cudaMemcpyDeviceToHost);
	if (ret != CUDA_SUCCESS) {
...
static int devmem_validate_token(unsigned char *rxbuf, struct memory_buffer *mem,
				 struct cmsghdr *cm, int rep, __u64 *tot_recv)
{
	...
	if (mem->provider == MEMORY_PROVIDER_CUDA) {
		ret = devmem_validate_cuda(rxbuf, mem, dmabuf_cmsg->frag_offset, start, dmabuf_cmsg->frag_size);
	} else {
		    ...
		    bool validate)
{
	...
	for (cm = CMSG_FIRSTHDR(&msg); cm; cm = CMSG_NXTHDR(&msg, cm)) {
		...
		if (validate) {
			ret = devmem_validate_token(rxbuf, mem, cm, rep, &tot_recv);
			if (ret < 0)
				return ret;
		}
